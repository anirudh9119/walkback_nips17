...
#import extensions
#import model
from util import  unzip, norm_weight, _p, itemlist,  load_params, create_log_dir,  save_params, create_svhn_streams  #ortho_weight
#import ipdb
...
def init_params(options):
    ...
    if use_conv:
        ...
        ''''
        params = get_layer('ff')[0](options, params, prefix='layer_1',nin=4*4*256, nout=1024,ortho=False)
        ...
        params = get_layer('ff')[0](options, params, prefix='layer_3',nin=256, nout=4*4*256,ortho=False)
        '''
        ...
        params = get_layer('ff')[0](options, params, prefix='layer_5',nin=1024, nout=4*4*256,ortho=False)
...
def join(a,b=None):
    if b==None:
        ...
        #return a+b #T.concatenate([a,b],axis=1)
...
def transition_operator(tparams, options, x, temperature):
    ...
    h1 = T.nnet.relu(batchnorm(fflayer(tparams, c3.flatten(2), options,prefix='layer_1', activ='lambda x: T.tanh(x)')))
    h2 = T.nnet.relu(batchnorm(fflayer(tparams, h1, options,prefix='layer_2', activ='lambda x: T.tanh(x)')))
    h3 = T.nnet.relu(batchnorm(fflayer(tparams, h2, options, prefix='layer_3', activ='lambda x: T.tanh(x)')))
    h4 = T.nnet.relu(batchnorm(fflayer(tparams, h3, options, prefix='layer_4', activ='lambda x: T.tanh(x)')))
    h5 = T.nnet.relu(batchnorm(fflayer(tparams, h4, options, prefix='layer_5', activ='lambda x: T.tanh(x)')))
    ...
    log_p_reverse = -0.5 * T.sum(T.abs_(x - mu) + 0.0 * (T.log(2 * np.pi) + T.log(sigma) + (x - mu) ** 2 / (sigma)),[1])
...
def train(args,
    ...
    model_id = '/data/lisatmp4/anirudhg/svhn_walk_back/walkback_'
    ...
    if args.dataset == 'MNIST':
        ...
        main_stream, train_stream, valid_stream, test_stream = create_svhn_streams(args.batch_size, args.batch_size)
        n_colors = 3
        spatial_width = 32
    ...
    if args.dataset != 'lsun' and args.dataset != 'celeba' and args.dataset != 'SVHN':
    ...
    f=transition_operator(tparams, model_options, x, temperature)
    for data in train_stream.get_epoch_iterator():
        t = data[0].reshape(100, 3 *32 *32)
        a,b,c,d,e,f,g,h, i,_,_ = f(t, 1.0)
        #print a
        ipdb.set_trace()
    '''
            ...
            if batch_index%200==0:
                ...
                for num_step in range(args.num_steps * args.meta_steps):
                    ...
                    if num_step == 0:
                        ...
                        plot_images(x_temp, model_dir + '/' + "batch_" + str(batch_index) + 'corrupted_' + 'epoch_' + str(count_sample) + '_time_step_' + str(num_step))
                    else:
