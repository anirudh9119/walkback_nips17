...
def train(args,
    ...
    model_id = '/data/lisatmp3/anirudhg/lsun_walkback/walkback_'
    model_dir = create_log_dir(args, model_id)
    model_id2 =  '../lsun_logs/walkback_'
    ...
    if args.dataset == 'MNIST':
        ...
        from load_lsun import load

        print "loading lsun data!"

        if args.dataset == "lsunsmall":
            dataset_train, dataset_test = load(args.batch_size, downsample=True)
            spatial_width=32
        else:
            dataset_train, dataset_test = load(args.batch_size, downsample=False)
            spatial_width=64
    ...
    scl = 3.0 #1./np.sqrt(np.mean((Xbatch-np.mean(Xbatch))**2))
    shft = 2.03#-np.mean(Xbatch*scl)
    ...
    for eidx in xrange(max_epochs):
        n_samples = 0
        for data in train_stream().next():  #train_stream.get_epoch_iterator():

            if args.dataset == 'lsun':
                data_use = (data.reshape(args.batch_size,3*64*64),)
            ...
            ud_start = time.time()
            data_run = data_use[0]
            data_run = data_run.astype(np.float32)
            data_run = np.divide(data_run, 256)
            data_run = data_run.astype(np.float32)
            temp_data_run = data_run
            ...
            gradient_updates_ = get_grads(temp_data_run, args.temperature)
            ...
            if batch_index%20==0:
                print 'Cost is this', batch_index, cost
            ...
            if batch_index%200==0:
                ...
                for num_step in range(args.num_steps * args.meta_steps):
                    if num_step == 0:
                        x_data, sampled, sampled_activation, sampled_preactivation = forward_diffusion(temp_data_run, temperature_forward)
                ...
                x_temp2 = temp_data_run.reshape(args.batch_size, n_colors, WIDTH, WIDTH)
